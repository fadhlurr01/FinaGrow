...
const Header: React.FC<HeaderProps> = ({ currentView, isMobileSidebarOpen, setIsMobileSidebarOpen, onLogout }) => {
  ...
  const storedName = localStorage.getItem('fms_active_user_name');
  const userName = storedName || (activeEmail ? activeEmail.split('@')[0] : 'User');
  const userEmail = activeEmail || '';
  const userPlan = state.subscription === 'Pro' ? 'Pro Plan' : 'Free Plan';
  
  // Load avatar image dynamically from localStorage or fallback
  const userAvatar = localStorage.getItem('fms_avatar_' + userEmail) || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(userName)}`;
